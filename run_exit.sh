...
# AlphaBeta every iteration (~2.5 min, ±1.4 pt) -- vk becomes the incumbent only if it wins more games
# (successive checkpoints swing 3-5 points, and early stopping on held-out loss does not track play, FINDINGS);
...
[ -f checkpoints_value/best.txt ] || echo "checkpoints_value/v$((first - 1)).pt 0" > checkpoints_value/best.txt
for k in $(seq "$first" "$last"); do
  read -r prev best_wins < checkpoints_value/best.txt
  V="vnet:$prev"
  echo "=== it$k incumbent $prev ($best_wins/4000 on the proxy)"
  ...
  out=$(run uv run python evaluate.py --player "vnet:checkpoints_value/v$k.pt" --opponent rab --games 4000) || exit 1
  echo "$out"
  wins=$(echo "$out" | sed -n 's/.*: \([0-9]*\)\/4000 wins.*/\1/p')
  if [ -n "$wins" ] && [ "$wins" -gt "$best_wins" ]; then
    echo "checkpoints_value/v$k.pt $wins" > checkpoints_value/best.txt; echo "=== it$k accepted: v$k is the new incumbent ($wins > $best_wins)"
